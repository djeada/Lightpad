...
FileDirTreeController::FileDirTreeController(FileDirTreeModel* model, QWidget* parent)
    : QObject(parent)
    , model(model)
    , parentWidget(parent)
{
    connect(model, &FileDirTreeModel::errorOccurred, this, &FileDirTreeController::showError);
}

void FileDirTreeController::handleNewFile(const QString& dirPath)
{
    bool ok;
    QString fileName = QInputDialog::getText(parentWidget, "New File",
        "Enter file name:", QLineEdit::Normal, "", &ok);
...
{
    bool ok;
    QString dirName = QInputDialog::getText(parentWidget, "New Directory",
        "Enter directory name:", QLineEdit::Normal, "", &ok);
...
void FileDirTreeController::handleRename(const QString& oldPath)
{
    ...
    QString newName = QInputDialog::getText(parentWidget, "Rename",
        "Enter new name:", QLineEdit::Normal, fileInfo.fileName(), &ok);
