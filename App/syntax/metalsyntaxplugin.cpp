...
QStringList MetalSyntaxPlugin::getPrimaryKeywords() {
  return {"void",     "bool",    "char",    "uchar",   "short",   "ushort",
          "int",      "uint",    "long",    "ulong",   "half",    "float",
          ...
          "texturecube", "texture1d_array", "texture2d_array",
          "texturecube_array", "texture2d_ms", "texture2d_ms_array",
          "depth2d",  "depthcube", "depth2d_array", "depthcube_array",
          "depth2d_ms", "depth2d_ms_array",
          "sampler",  "array",   "atomic_int", "atomic_uint"};
}
...
QStringList MetalSyntaxPlugin::getSecondaryKeywords() {
  return {"if",       "else",    "for",     "while",   "do",
          ...
          "enum",     "union",   "sizeof",  "typedef",
          "discard_fragment"};
}

QStringList MetalSyntaxPlugin::getTertiaryKeywords() {
  return {"true",       "false",       "NULL",       "nullptr",
          "constant",   "device",      "threadgroup","threadgroup_imageblock",
          "thread",     "object_data", "ray_data",   "patch_control_point",
          "kernel",     "vertex",      "fragment",   "tile",
          "visible",    "stitchable",  "object",     "mesh",
          "in",         "out",         "inout",
          "[[vertex_id]]", "[[instance_id]]", "[[position]]",
          "[[front_facing]]", "[[point_coord]]", "[[sample_id]]",
          ...
          "[[threads_per_grid]]", "[[threadgroup_position_in_grid]]",
          "[[threadgroups_per_grid]]", "[[dispatch_threads_per_threadgroup]]",
          "[[color]]",  "[[depth]]",   "[[stencil]]",
