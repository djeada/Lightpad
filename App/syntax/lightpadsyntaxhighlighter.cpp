...
HighlightingRule::HighlightingRule(QRegularExpression pattern,
...
LightpadSyntaxHighlighter::LightpadSyntaxHighlighter(
    ...
    QRegularExpression commentEndExpression, QTextDocument *parent)
    : QSyntaxHighlighter(parent), highlightingRules(highlightingRules),
      commentStartExpression(commentStartExpression),
      commentEndExpression(commentEndExpression), m_firstVisibleBlock(0),
...
static void loadHighlightingRules(QVector<HighlightingRule> &highlightingRules,
                                  const QStringList &keywordPatterns_0,
  ...
  for (auto &pattern : keywordPatterns_0)
    highlightingRules.append(
        HighlightingRule(QRegularExpression(pattern), keywordFormat));
  ...
  for (auto &pattern : keywordPatterns_1)
    highlightingRules.append(
        HighlightingRule(QRegularExpression(pattern), keywordFormat_1));
  ...
  for (auto &pattern : keywordPatterns_2)
    highlightingRules.append(
        HighlightingRule(QRegularExpression(pattern), keywordFormat_2));
  ...
  highlightingRules.append(HighlightingRule(
      QRegularExpression(QStringLiteral("\\bQ[A-Za-z]+\\b")), classFormat));
  ...
  highlightingRules.append(
      HighlightingRule(singleLineComment, singleLineCommentFormat));

  if (!searchKeyword.isEmpty()) {
    QTextCharFormat searchFormat;
    searchFormat.setBackground(QColor("#646464"));
    highlightingRules.append(HighlightingRule(
        QRegularExpression(searchKeyword,
                           QRegularExpression::CaseInsensitiveOption),
        searchFormat));
  }
}

QVector<HighlightingRule> highlightingRulesCpp(Theme colors,
                                               const QString &searchKeyword) {
  QVector<HighlightingRule> highlightingRules;
  ...
  if (!keywordPatterns_0.isEmpty() && !keywordPatterns_1.isEmpty() &&
      !keywordPatterns_2.isEmpty())
    loadHighlightingRules(highlightingRules, keywordPatterns_0,
                          keywordPatterns_1, keywordPatterns_2, searchKeyword,
...
QVector<HighlightingRule> highlightingRulesJs(Theme colors,
                                              const QString &searchKeyword) {
  QVector<HighlightingRule> highlightingRules;
  QStringList keywordPatterns_0;
  QStringList keywordPatterns_1;
...
QVector<HighlightingRule> highlightingRulesPy(Theme colors,
                                              const QString &searchKeyword) {
  QVector<HighlightingRule> highlightingRules;
  QStringList keywordPatterns_0;
  QStringList keywordPatterns_1;
