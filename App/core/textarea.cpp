...
void TextArea::keyPressEvent(QKeyEvent* keyEvent)
{
    ...
    if (m_vimMode && m_vimMode->isEnabled() && m_vimMode->processKeyEvent(keyEvent)) {
        return;
    }
...
void TextArea::mousePressEvent(QMouseEvent* event)
{
    ...
    if (m_multiCursor && m_multiCursor->hasMultipleCursors() && !(event->modifiers() & Qt::ControlModifier)) {
        clearExtraCursors();
    }
