...
void CompletionProviderRegistry::registerProvider(std::shared_ptr<ICompletionProvider> provider)
{
    if (!provider) {
        Logger::instance().warning("Attempted to register null completion provider");
        return;
    }
    
    QString providerId = provider->id();
    if (providerId.isEmpty()) {
        Logger::instance().warning("Attempted to register completion provider with empty ID");
        return;
    }
    ...
    Logger::instance().info(
        QString("Registered completion provider '%1' (%2)")
            .arg(providerId)
...
std::shared_ptr<ICompletionProvider> CompletionProviderRegistry::getProvider(
...
QList<std::shared_ptr<ICompletionProvider>> CompletionProviderRegistry::providersForLanguage(
...
) const
{
    ...
    for (auto it = m_providers.begin(); it != m_providers.end(); ++it) {
        ...
        if (provider && provider->isEnabled() && provider->supportsLanguage(languageId)) {
...
QList<std::shared_ptr<ICompletionProvider>> CompletionProviderRegistry::allProviders() const
{
...
QStringList CompletionProviderRegistry::allTriggerCharacters(const QString& languageId) const
{
...
bool CompletionProviderRegistry::hasProvidersForLanguage(const QString& languageId) const
{
    for (auto it = m_providers.begin(); it != m_providers.end(); ++it) {
        auto& provider = it.value();
        if (provider && provider->isEnabled() && provider->supportsLanguage(languageId)) {
