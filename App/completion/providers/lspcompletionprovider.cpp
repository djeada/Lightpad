    ...
    , m_client(client)
{
    if (m_client) {
        connect(m_client, &LspClient::completionReceived,
                this, &LspCompletionProvider::onCompletionReceived);
    }
}
...
{
    if (m_client) {
        disconnect(m_client, &LspClient::completionReceived,
                   this, &LspCompletionProvider::onCompletionReceived);
    }
...
CompletionItem LspCompletionProvider::convertItem(const LspCompletionItem& lspItem) const
{
    CompletionItem item;
    item.label = lspItem.label;
    item.insertText = lspItem.insertText.isEmpty() ? lspItem.label : lspItem.insertText;
