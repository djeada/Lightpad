    ...
    : QObject(nullptr)
{
    ...
    QString userPlugins = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/plugins";
...
bool PluginManager::loadPlugin(const QString& filePath)
{
    ...
    if (syntaxPlugin) {
        ...
        LOG_INFO(QString("Loaded syntax plugin for language: %1").arg(syntaxPlugin->languageId()));
    }
...
int PluginManager::loadAllPlugins()
{
    ...
    LOG_INFO(QString("Loaded %1 of %2 discovered plugins").arg(loaded).arg(plugins.size()));
    return loaded;
}
...
QMap<QString, IPlugin*> PluginManager::allPlugins() const
{
    return m_plugins;
}
...
ISyntaxPlugin* PluginManager::syntaxPluginForExtension(const QString& extension) const
{
