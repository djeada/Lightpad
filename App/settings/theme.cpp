...
Theme::Theme()
    // Editor colors
    : backgroundColor(QColor("#0d1117"))
    , foregroundColor(QColor("#e6edf3"))
    , highlightColor(QColor("#1a2230"))
    , lineNumberAreaColor(QColor("#0d1117"))
    ...
    , surfaceColor(QColor("#161b22"))
    , surfaceAltColor(QColor("#1c2128"))
    , borderColor(QColor("#30363d"))
    , hoverColor(QColor("#21262d"))
    , pressedColor(QColor("#2d333b"))
    , accentColor(QColor("#58a6ff"))
    , accentSoftColor(QColor("#1f3a5f"))
    , successColor(QColor("#3fb950"))
    , warningColor(QColor("#d29922"))
    , errorColor(QColor("#f85149"))
{
}
...
void Theme::read(const QJsonObject& json)
{
    ...
    // Modern UI tokens
    if (themeObject.contains("surfaceColor") && themeObject["surfaceColor"].isString())
        surfaceColor = QColor(themeObject["surfaceColor"].toString());

    if (themeObject.contains("surfaceAltColor") && themeObject["surfaceAltColor"].isString())
        surfaceAltColor = QColor(themeObject["surfaceAltColor"].toString());

    if (themeObject.contains("borderColor") && themeObject["borderColor"].isString())
        borderColor = QColor(themeObject["borderColor"].toString());

    if (themeObject.contains("hoverColor") && themeObject["hoverColor"].isString())
        hoverColor = QColor(themeObject["hoverColor"].toString());

    if (themeObject.contains("pressedColor") && themeObject["pressedColor"].isString())
        pressedColor = QColor(themeObject["pressedColor"].toString());

    if (themeObject.contains("accentColor") && themeObject["accentColor"].isString())
        accentColor = QColor(themeObject["accentColor"].toString());

    if (themeObject.contains("accentSoftColor") && themeObject["accentSoftColor"].isString())
        accentSoftColor = QColor(themeObject["accentSoftColor"].toString());

    if (themeObject.contains("successColor") && themeObject["successColor"].isString())
        successColor = QColor(themeObject["successColor"].toString());

    if (themeObject.contains("warningColor") && themeObject["warningColor"].isString())
        warningColor = QColor(themeObject["warningColor"].toString());

    if (themeObject.contains("errorColor") && themeObject["errorColor"].isString())
