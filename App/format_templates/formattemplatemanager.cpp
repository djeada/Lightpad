...
FormatTemplateManager::FormatTemplateManager()
    : QObject(nullptr)
{
}
...
bool FormatTemplateManager::loadBuiltInTemplates()
{
    ...
    QStringList searchPaths = {
        ...
        QStandardPaths::locate(QStandardPaths::AppDataLocation, "format_templates/format_templates.json")
    };
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        LOG_ERROR(QString("Failed to open format templates file: %1").arg(filePath));
        return false;
    }
    ...
    if (parseError.error != QJsonParseError::NoError) {
        LOG_ERROR(QString("Failed to parse format templates: %1").arg(parseError.errorString()));
        return false;
    }
    ...
    LOG_INFO(QString("Loaded %1 built-in format templates from %2").arg(m_templates.size()).arg(filePath));
    return true;
}
...
bool FormatTemplateManager::loadUserTemplates()
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        LOG_WARNING(QString("Failed to open user format templates file: %1").arg(userTemplatesPath));
        return false;
    }
    ...
    if (parseError.error != QJsonParseError::NoError) {
        LOG_WARNING(QString("Failed to parse user format templates: %1").arg(parseError.errorString()));
        return false;
    }
...
FormatTemplate FormatTemplateManager::parseTemplate(const QJsonObject& obj) const
{
    FormatTemplate tmpl;
...
QList<FormatTemplate> FormatTemplateManager::getTemplatesForExtension(const QString& extension) const
{
...
QString FormatTemplateManager::getConfigDirForFile(const QString& filePath) const
{
    QFileInfo fileInfo(filePath);
    return fileInfo.absoluteDir().path() + "/.lightpad";
}

QString FormatTemplateManager::getConfigFileForDir(const QString& dirPath) const
{
    return dirPath + "/format_config.json";
}

bool FormatTemplateManager::loadAssignmentsFromDir(const QString& dirPath) const
{
    ...
    if (parseError.error != QJsonParseError::NoError) {
        LOG_WARNING(QString("Failed to parse format config: %1").arg(parseError.errorString()));
        return false;
    }
    ...
    m_loadedConfigDirs.insert(configDir);
    LOG_INFO(QString("Loaded %1 format assignments from %2").arg(assignments.size()).arg(configFile));
    return true;
}
...
bool FormatTemplateManager::saveAssignmentsToDir(const QString& dirPath) const
{
    ...
    LOG_INFO(QString("Saved %1 format assignments to %2").arg(assignments.size()).arg(configFile));
    return true;
}

FileFormatAssignment FormatTemplateManager::getAssignmentForFile(const QString& filePath) const
{
...
bool FormatTemplateManager::assignTemplateToFile(const QString& filePath,
...
QString FormatTemplateManager::substituteVariables(const QString& input, const QString& filePath)
{
...
QPair<QString, QStringList> FormatTemplateManager::buildCommand(const QString& filePath) const
{
