...
void I18n::initialize(QCoreApplication* app)
{
    ...
    LOG_INFO(QString("I18n initialized with language: %1").arg(m_currentLanguage));
}
...
{
    QLocale locale;
    QString lang = locale.name().left(2);  // Get first two characters (e.g., "en" from "en_US")
    return lang;
}
...
QString I18n::translationsDirectory() const
{
    ...
    // User data directory
    paths << QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + "/translations";
...
void I18n::loadAvailableLanguages()
{
    ...
    LOG_DEBUG(QString("Found %1 available languages").arg(m_availableLanguages.size()));
}
...
bool I18n::loadTranslation(const QString& languageCode)
{
    ...
    if (m_translator->load(appTransFile)) {
        ...
        if (m_translator->load(":/translations/lightpad_" + languageCode + ".qm")) {
            m_app->installTranslator(m_translator);
            LOG_DEBUG(QString("Loaded translation from resources: %1").arg(languageCode));
        } else {
            LOG_WARNING(QString("Could not load translation for: %1").arg(languageCode));
            return false;
        }
    }
