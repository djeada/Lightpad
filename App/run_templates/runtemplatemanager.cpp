...
RunTemplateManager::RunTemplateManager()
    : QObject(nullptr)
{
}
...
bool RunTemplateManager::loadBuiltInTemplates()
{
    ...
    QStringList searchPaths = {
        ...
        QStandardPaths::locate(QStandardPaths::AppDataLocation, "run_templates/run_templates.json")
    };
    ...
    if (parseError.error != QJsonParseError::NoError) {
        LOG_ERROR(QString("Failed to parse run templates: %1").arg(parseError.errorString()));
        return false;
    }
    ...
    LOG_INFO(QString("Loaded %1 built-in templates from %2").arg(m_templates.size()).arg(filePath));
    return true;
}
...
bool RunTemplateManager::loadUserTemplates()
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        LOG_WARNING(QString("Failed to open user templates file: %1").arg(userTemplatesPath));
        return false;
    }
    ...
    if (parseError.error != QJsonParseError::NoError) {
        LOG_WARNING(QString("Failed to parse user templates: %1").arg(parseError.errorString()));
        return false;
    }
...
QList<RunTemplate> RunTemplateManager::getTemplatesForExtension(const QString& extension) const
{
...
bool RunTemplateManager::loadAssignmentsFromDir(const QString& dirPath)
{
    ...
    if (parseError.error != QJsonParseError::NoError) {
        LOG_WARNING(QString("Failed to parse run config: %1").arg(parseError.errorString()));
        return false;
    }
    ...
    m_loadedConfigDirs.insert(configDir);
    LOG_INFO(QString("Loaded %1 assignments from %2").arg(assignments.size()).arg(configFile));
    return true;
}
...
bool RunTemplateManager::saveAssignmentsToDir(const QString& dirPath)
{
    ...
    LOG_INFO(QString("Saved %1 assignments to %2").arg(assignments.size()).arg(configFile));
    return true;
}

FileTemplateAssignment RunTemplateManager::getAssignmentForFile(const QString& filePath) const
{
...
bool RunTemplateManager::assignTemplateToFile(const QString& filePath,
                                               const QString& templateId,
                                               const QStringList& customArgs,
                                               const QMap<QString, QString>& customEnv)
{
    FileTemplateAssignment assignment;
...
QString RunTemplateManager::substituteVariables(const QString& input, const QString& filePath)
{
...
QPair<QString, QStringList> RunTemplateManager::buildCommand(const QString& filePath) const
{
...
QMap<QString, QString> RunTemplateManager::getEnvironment(const QString& filePath) const
{
    FileTemplateAssignment assignment = getAssignmentForFile(filePath);
    ...
    for (auto it = assignment.customEnv.begin(); it != assignment.customEnv.end(); ++it) {
        env[it.key()] = substituteVariables(it.value(), filePath);
    }
