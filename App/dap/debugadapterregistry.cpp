...
public:
    DebugAdapterConfig config() const override {
        ...
        cfg.languages = QStringList() << "cpp" << "c" << "fortran" << "rust" << "go" << "asm";
    ...
    QString statusMessage() const override {
        ...
        return QString("Ready - %1%2").arg(firstLine, capabilities.isEmpty() ? "" : QString(" (%1)").arg(capabilities));
    }
    ...
    QJsonObject createCoreDumpConfig(const QString& coreDumpPath,
        ...
        QJsonObject config;
        config["name"] = QString("Analyze core dump: %1").arg(QFileInfo(coreDumpPath).fileName());
        config["type"] = "cppdbg";
        config["request"] = "launch";
    ...
    QString installCommand() const override {
        ...
        if (QFileInfo::exists("/usr/bin/apt") || QFileInfo::exists("/usr/bin/apt-get")) {
...
private:
    ...
    QString getGdbCapabilities(const QString& gdbPath) const {
        ...
        if (proc.waitForFinished(3000)) {
            QString output = QString::fromUtf8(proc.readAllStandardOutput() + proc.readAllStandardError());
