    ...
    , m_client(std::make_unique<DapClient>(this))
{
    ...
    connect(m_client.get(), &DapClient::stateChanged,
            this, &DebugSession::onClientStateChanged);
    connect(m_client.get(), &DapClient::stopped,
            this, &DebugSession::onClientStopped);
    connect(m_client.get(), &DapClient::terminated,
            this, &DebugSession::onClientTerminated);
    connect(m_client.get(), &DapClient::output,
            this, &DebugSession::onClientOutput);
    connect(m_client.get(), &DapClient::error,
            this, &DebugSession::onClientError);
}
                         ...
                         std::shared_ptr<IDebugAdapter> adapter)
{
    ...
    LOG_INFO(QString("Started debug session %1 with adapter %2")
             .arg(m_id).arg(adapterConfig.name));
        ...
        case DapClient::State::Ready:
            ...
            if (m_state == State::Starting) {
                if (m_configuration.request == "attach") {
                    ...
                    // Launch
                    m_client->launch(m_configuration.program,
...
QString DebugSessionManager::startSession(const DebugConfiguration& config)
{
    ...
    auto adapters = DebugAdapterRegistry::instance().adaptersForLanguage(config.type);
    if (adapters.isEmpty()) {
    ...
    if (!adapter) {
        LOG_ERROR(QString("No available debug adapter for type: %1").arg(config.type));
        return {};
    }
                                          ...
                                          std::shared_ptr<IDebugAdapter> adapter)
{
    ...
    connect(session, &DebugSession::stateChanged,
            this, &DebugSessionManager::onSessionStateChanged);
    connect(session, &DebugSession::terminated,
            this, &DebugSessionManager::onSessionTerminated);
    connect(session, &DebugSession::stopped,
            this, [this, sessionId](const DapStoppedEvent& event) {
                emit sessionStopped(sessionId, event);
            });
    connect(session, &DebugSession::error,
            this, [this, sessionId](const QString& message) {
...
{
    DebugConfiguration config = DebugConfigurationManager::instance().createQuickConfig(filePath);
    if (config.name.isEmpty()) {
        LOG_ERROR(QString("Could not create debug configuration for: %1").arg(filePath));
        return {};
    }
...
void DebugSessionManager::stopSession(const QString& sessionId, bool terminate)
{
...
void DebugSessionManager::onSessionTerminated()
{
    ...
    QMetaObject::invokeMethod(this, [this, sessionId, senderSession]() {
