...
public:
     ...
     */
    bool start(const DebugConfiguration& config, std::shared_ptr<IDebugAdapter> adapter);
    ...
    void continueExecution() { if (m_client) m_client->continueExecution(); }
    void pause() { if (m_client) m_client->pause(); }
    void stepOver() { if (m_client) m_client->stepOver(0); }
    void stepInto() { if (m_client) m_client->stepInto(0); }
    void stepOut() { if (m_client) m_client->stepOut(0); }
