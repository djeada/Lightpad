...
void DebugSettings::initialize(const QString &workspaceFolder) {
  ...
  LOG_INFO(
      QString("Debug settings initialized in: %1").arg(debugSettingsDir()));
}

QString DebugSettings::workspaceFolder() const { return m_workspaceFolder; }
...
void DebugSettings::ensureDirectoryExists() {
  ...
  if (!dir.exists()) {
    dir.mkpath(".");
    LOG_INFO(QString("Created debug settings directory: %1")
...
void DebugSettings::createDefaultLaunchConfig() {
  ...
  config["_comment"] = "Debug launch configurations. Edit this file to add "
  ...
  exampleCompound["_comment"] =
      "Example compound configuration for debugging multiple targets";
...
QJsonObject DebugSettings::generalSettings() const { return m_generalSettings; }

void DebugSettings::setGeneralSetting(const QString &key,
...
QJsonObject DebugSettings::adapterSettings() const { return m_adapterSettings; }

void DebugSettings::setAdapterSettings(const QString &adapterId,
...
QString
DebugSettings::defaultAdapterForExtension(const QString &extension) const {
  QJsonObject defaults = m_adapterSettings["defaultAdapters"].toObject();
  return defaults[extension].toString();
}

void DebugSettings::setDefaultAdapterForExtension(const QString &extension,
...
bool DebugSettings::writeJsonFile(const QString &path,
...
QJsonObject DebugSettings::readJsonFile(const QString &path) {
  ...
  if (error.error != QJsonParseError::NoError) {
    LOG_ERROR(
        QString("JSON parse error in %1: %2").arg(path, error.errorString()));
    return QJsonObject();
  }
