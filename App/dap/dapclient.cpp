...
bool DapClient::start(const QString& program, const QStringList& arguments)
{
    ...
    connect(m_process, &QProcess::readyReadStandardOutput,
            this, &DapClient::onReadyReadStandardOutput);
    connect(m_process, &QProcess::readyReadStandardError,
            this, &DapClient::onReadyReadStandardError);
...
DapClient::State DapClient::state() const
{
    return m_state;
}
...
void DapClient::getStackTrace(int threadId, int startFrame, int levels)
{
    ...
    if (startFrame > 0) args["startFrame"] = startFrame;
                         ...
                         const QString& context)
{
    ...
    if (frameId > 0) args["frameId"] = frameId;
...
void DapClient::sendRequest(const QString& command, const QJsonObject& arguments, int seq)
{
    QJsonObject message;
    message["seq"] = seq;
...
void DapClient::sendResponse(int requestSeq, const QString& command, bool success,
...
void DapClient::onReadyReadStandardOutput()
{
    ...
    while (iterations < maxIterations) {
        ...
        if (parseError.error != QJsonParseError::NoError) {
            LOG_ERROR(QString("Failed to parse DAP message: %1").arg(parseError.errorString()));
            continue;
        }
...
void DapClient::handleMessage(const QJsonObject& message)
{
    ...
    if (type == "response") {
        ...
        handleResponse(requestSeq, command, success,
...
void DapClient::handleResponse(int requestSeq, const QString& command, bool success,
