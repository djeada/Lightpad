...
struct DapSourceBreakpoint {
    ...
    QJsonObject toJson() const {
        ...
        if (!logMessage.isEmpty()) obj["logMessage"] = logMessage;
        return obj;
    }
};
...
struct DapStoppedEvent {
    ...
    static DapStoppedEvent fromJson(const QJsonObject& obj) {
        ...
        if (reasonStr == "step") evt.reason = DapStoppedReason::Step;
        else if (reasonStr == "breakpoint") evt.reason = DapStoppedReason::Breakpoint;
        else if (reasonStr == "exception") evt.reason = DapStoppedReason::Exception;
        else if (reasonStr == "pause") evt.reason = DapStoppedReason::Pause;
        else if (reasonStr == "entry") evt.reason = DapStoppedReason::Entry;
        else if (reasonStr == "goto") evt.reason = DapStoppedReason::Goto;
        else if (reasonStr == "function breakpoint") evt.reason = DapStoppedReason::FunctionBreakpoint;
        else if (reasonStr == "data breakpoint") evt.reason = DapStoppedReason::DataBreakpoint;
        else if (reasonStr == "instruction breakpoint") evt.reason = DapStoppedReason::InstructionBreakpoint;
...
signals:
    ...
    // Breakpoint events
    void breakpointsSet(const QString& sourcePath, const QList<DapBreakpoint>& breakpoints);
    void breakpointChanged(const DapBreakpoint& breakpoint, const QString& reason);
    ...
    void stackTraceReceived(int threadId, const QList<DapStackFrame>& frames, int totalFrames);
    void scopesReceived(int frameId, const QList<DapScope>& scopes);
    void variablesReceived(int variablesReference, const QList<DapVariable>& variables);
...
private:
    void sendRequest(const QString& command, const QJsonObject& arguments, int seq);
    ...
    void handleReverseRequest(int seq, const QString& command, const QJsonObject& arguments);
