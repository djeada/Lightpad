...
Breakpoint BreakpointManager::toggleBreakpoint(const QString& filePath, int line)
{
...
int BreakpointManager::addBreakpoint(const Breakpoint& bp)
{
    ...
    LOG_DEBUG(QString("Added breakpoint %1 at %2:%3")
              .arg(newBp.id).arg(newBp.filePath).arg(newBp.line));
...
void BreakpointManager::removeBreakpoint(int id)
{
    ...
    LOG_DEBUG(QString("Removed breakpoint %1 at %2:%3")
              .arg(id).arg(bp.filePath).arg(bp.line));
...
QList<Breakpoint> BreakpointManager::breakpointsForFile(const QString& filePath) const
{
...
Breakpoint BreakpointManager::breakpointAt(const QString& filePath, int line) const
{
    QList<int> ids = m_fileBreakpoints.value(filePath);
    for (int id : ids) {
        const Breakpoint& bp = m_breakpoints.value(id);
...
void BreakpointManager::updateVerification(const QString& filePath,
...
bool BreakpointManager::loadFromFile(const QString& filePath)
{
    ...
    if (error.error != QJsonParseError::NoError) {
        LOG_ERROR(QString("Failed to parse breakpoints file: %1").arg(error.errorString()));
        return false;
    }
