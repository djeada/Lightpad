...
AccessibilityManager::AccessibilityManager()
    : QObject(nullptr)
    , m_currentProfile(Profile::Default)
    , m_highContrastEnabled(false)
    , m_fontScale(1.0)
    , m_reducedMotionEnabled(false)
...
void AccessibilityManager::applyProfile(Profile profile)
{
    ...
    LOG_INFO(QString("Applied accessibility profile: %1").arg(static_cast<int>(profile)));
}
...
void AccessibilityManager::setHighContrastEnabled(bool enabled)
{
    if (m_highContrastEnabled != enabled) {
        ...
        LOG_DEBUG(QString("High contrast: %1").arg(enabled ? "enabled" : "disabled"));
    }
}
...
double AccessibilityManager::fontScale() const
{
    return m_fontScale;
}
...
void AccessibilityManager::resetFontScale()
{
    setFontScale(1.0);
}
...
void AccessibilityManager::setReducedMotionEnabled(bool enabled)
{
    if (m_reducedMotionEnabled != enabled) {
        ...
        LOG_DEBUG(QString("Reduced motion: %1").arg(enabled ? "enabled" : "disabled"));
    }
}
...
void AccessibilityManager::setScreenReaderEnabled(bool enabled)
{
    if (m_screenReaderEnabled != enabled) {
        ...
        LOG_DEBUG(QString("Screen reader mode: %1").arg(enabled ? "enabled" : "disabled"));
    }
}
...
void AccessibilityManager::loadSettings()
{
    ...
    m_highContrastEnabled = settings.getValue("accessibility/highContrast", false).toBool();
    m_fontScale = settings.getValue("accessibility/fontScale", 1.0).toDouble();
    m_reducedMotionEnabled = settings.getValue("accessibility/reducedMotion", false).toBool();
    m_screenReaderEnabled = settings.getValue("accessibility/screenReader", false).toBool();
...
void AccessibilityManager::saveSettings()
{
    ...
    settings.setValue("accessibility/profile", static_cast<int>(m_currentProfile));
}
