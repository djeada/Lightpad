...
PdfViewer::PdfViewer(QWidget *parent)
    : QWidget(parent), m_document(nullptr), m_pdfView(nullptr),
      m_toolbar(nullptr), m_pageLabel(nullptr), m_zoomLabel(nullptr),
      m_pageSpinBox(nullptr), m_zoomFactor(1.0) {
  setupUi();
}
...
void PdfViewer::nextPage() {
  if (m_pageSpinBox && m_document &&
      m_pageSpinBox->value() < m_document->pageCount()) {
    m_pageSpinBox->setValue(m_pageSpinBox->value() + 1);
  }
}
...
void PdfViewer::updatePageLabel() {
  ...
  m_pageLabel->setText(
      QString("Page %1 of %2").arg(currentPage).arg(m_document->pageCount()));
}
