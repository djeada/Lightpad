...
ColorPicker::ColorPicker(Theme theme, MainWindow *parent)
    : QDialog(nullptr), ui(new Ui::ColorPicker), parentWindow(parent) {
  setWindowFlag(Qt::Popup);
  ui->setupUi(this);

  ui->buttonBackground->setStyleSheet(
      buttonStyleSheet + "background: " + theme.backgroundColor.name() + ";");
  ui->buttonClases->setStyleSheet(
      buttonStyleSheet + "background: " + theme.classFormat.name() + ";");
  ui->buttonComments->setStyleSheet(buttonStyleSheet + "background: " +
                                    theme.singleLineCommentFormat.name() + ";");
  ui->buttonQuotations->setStyleSheet(
      buttonStyleSheet + "background: " + theme.quotationFormat.name() + ";");
  ui->buttonFont->setStyleSheet(
      buttonStyleSheet + "background: " + theme.foregroundColor.name() + ";");
  ui->buttonFunctions->setStyleSheet(
      buttonStyleSheet + "background: " + theme.functionFormat.name() + ";");
  ui->buttonKeywords1->setStyleSheet(
      buttonStyleSheet + "background: " + theme.keywordFormat_0.name() + ";");
  ui->buttonKeywords2->setStyleSheet(
      buttonStyleSheet + "background: " + theme.keywordFormat_1.name() + ";");
  ui->buttonKeywords3->setStyleSheet(
      buttonStyleSheet + "background: " + theme.keywordFormat_2.name() + ";");
  ui->buttonNumbers->setStyleSheet(
      buttonStyleSheet + "background: " + theme.numberFormat.name() + ";");
...
void ColorPicker::setParentWindow(MainWindow *window) { parentWindow = window; }
