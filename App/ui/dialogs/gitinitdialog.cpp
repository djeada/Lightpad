...
GitInitDialog::GitInitDialog(const QString& projectPath, QWidget* parent)
    ...
    , m_initialCommitCheckbox(nullptr)
    , m_gitIgnoreCheckbox(nullptr)
    , m_remoteEdit(nullptr)
...
void GitInitDialog::setupUI()
{
    ...
    QLabel* subtitleLabel = new QLabel(tr("This folder is not a Git repository. Initialize one to start tracking changes."), this);
    ...
    connect(m_browseButton, &QPushButton::clicked, this, &GitInitDialog::onBrowseClicked);
    ...
    m_initialCommitCheckbox->setToolTip(tr("Create an empty initial commit after initialization"));
    ...
    m_remoteEdit->setPlaceholderText(tr("https://github.com/username/repository.git"));
    ...
    connect(m_cancelButton, &QPushButton::clicked, this, &GitInitDialog::onCancelClicked);
    buttonLayout->addWidget(m_cancelButton);
    
    m_initButton = new QPushButton(tr("Initialize"), this);
    m_initButton->setFixedWidth(120);
    m_initButton->setDefault(true);
    connect(m_initButton, &QPushButton::clicked, this, &GitInitDialog::onInitClicked);
...
void GitInitDialog::onInitClicked()
{
    ...
    if (path.isEmpty()) {
        QMessageBox::warning(this, tr("Error"), tr("Please specify a repository path."));
        return;
    }
...
void GitInitDialog::onCancelClicked()
{
    reject();
}
