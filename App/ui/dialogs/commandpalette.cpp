...
void CommandPalette::setupUI()
{
    ...
    connect(m_resultsList, &QListWidget::itemClicked, this, &CommandPalette::onItemClicked);

    // Install event filter for keyboard navigation
    m_searchBox->installEventFilter(this);

    setStyleSheet("CommandPalette { background: #171c24; border: 1px solid #2a3241; border-radius: 8px; }");
}
...
void CommandPalette::registerAction(QAction* action, const QString& category)
{
    ...
    CommandItem item;
    item.id = action->objectName().isEmpty() ? action->text() : action->objectName();
    item.name = category.isEmpty() ? action->text() : category + ": " + action->text();
