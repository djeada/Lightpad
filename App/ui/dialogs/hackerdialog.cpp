...
void HackerDialog::paintEvent(QPaintEvent *) {
  ...
  // Subtle top edge highlight (1px lighter band)
...
QString HackerDialog::checkBoxStyle() const {
  ...
  return QString(
             ...
             "  border-color: %5;"
             "}")
      .arg(c.textPrimary.name(), c.borderDefault.name(),
           c.surfaceSunken.name(), c.accentPrimary.name(),
...
QString HackerDialog::tableWidgetStyle() const {
  ...
  return QString(
             ...
             "}")
      .arg(c.surfaceBase.name(), c.surfaceRaised.name(),
           c.textPrimary.name(), c.borderDefault.name(),
           c.accentSoft.name(), c.textMuted.name());
}
...
QString HackerDialog::tabWidgetStyle() const {
  ...
  return QString(
             ...
             "  background: %7;"
             "}")
      .arg(c.borderDefault.name(), c.surfaceBase.name(),
           c.surfaceRaised.name(), c.textMuted.name(),
           c.textPrimary.name(), c.accentPrimary.name(),
           c.treeHoverBg.name());
}
...
void HackerDialog::stylePrimaryButton(QPushButton *btn) {
  if (btn) {
    const auto &c = colors();
    btn->setStyleSheet(
        buttonStyle(c.btnPrimaryBg, c.btnPrimaryFg, c.btnPrimaryHover,
                    c.btnPrimaryBg));
  }
}
...
void HackerDialog::styleSecondaryButton(QPushButton *btn) {
  if (btn) {
    const auto &c = colors();
    btn->setStyleSheet(
        buttonStyle(c.btnSecondaryBg, c.btnSecondaryFg, c.btnSecondaryHover,
                    c.borderDefault));
  }
}
...
void HackerDialog::styleDangerButton(QPushButton *btn) {
  if (btn) {
    const auto &c = colors();
    btn->setStyleSheet(
        buttonStyle(c.btnDangerBg, c.btnDangerFg, c.btnDangerHover,
                    c.btnDangerBg));
  }
}
...
void HackerDialog::styleSubduedLabel(QLabel *label) {
  if (label)
    label->setStyleSheet(
        QString("color: %1;").arg(colors().textMuted.name()));
}
