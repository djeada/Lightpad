...
GitDiffDialog::GitDiffDialog(GitIntegration* git,
    ...
    , m_changeCounterLabel(nullptr)
    , m_modeSelector(nullptr)
    , m_wrapToggle(nullptr)
    ...
    , m_theme(theme)
{
    ...
    if (m_target == DiffTarget::File) {
        ...
        m_summaryText = tr("Diff • %1").arg(info.fileName().isEmpty() ? m_targetId : info.fileName());
    } else {
...
void GitDiffDialog::setDiffText(const QString& diffText)
{
    ...
    if (m_changeCounterLabel) {
        m_changeCounterLabel->setText(tr("0 changes • +%1 -%2")
            .arg(m_totalAdded)
            .arg(m_totalDeleted));
    }
...
{
    if (m_diffView) {
        m_diffView->setLineWrapMode(enabled ? QTextEdit::WidgetWidth : QTextEdit::NoWrap);
    }
}
...
void GitDiffDialog::onPrevChange()
{
    ...
    m_currentChange = (m_currentChange - 1 + m_changeBlocks.size()) % m_changeBlocks.size();
...
void GitDiffDialog::buildUi()
{
    ...
    m_diffView->setTabStopDistance(QFontMetrics(m_diffView->font()).horizontalAdvance(' ') * 4);
    ...
    connect(m_wrapToggle, &QCheckBox::toggled, this, &GitDiffDialog::onWrapToggled);
    ...
    connect(m_findPrevButton, &QPushButton::clicked, [this]() { performSearch(true); });
    connect(m_findNextButton, &QPushButton::clicked, [this]() { performSearch(false); });
...
void GitDiffDialog::applyTheme(const Theme& theme)
{
    ...
    for (QPushButton* button : {m_prevButton, m_nextButton, m_findPrevButton, m_findNextButton, m_copyButton}) {
        if (button) {
...
void GitDiffDialog::updateDiffPresentation()
{
    ...
    const QString diffSource = (m_viewMode == DiffViewMode::Word && !m_wordDiffText.isEmpty())
        ? m_wordDiffText
        : m_diffText;
...
void GitDiffDialog::rebuildSplit()
{
    ...
    for (const auto& line : m_lines) {
        QString escaped = htmlEscape(line.content);
        if (line.prefix == '+') {
            html += "<tr><td class=\"gutter\"></td><td class=\"add\">+" + escaped + "</td></tr>";
        } else if (line.prefix == '-') {
            html += "<tr><td class=\"del\">-" + escaped + "</td><td class=\"gutter\"></td></tr>";
        } else if (line.prefix == '@' || line.prefix == 'd' || line.prefix == 'i' || line.prefix == 'h') {
            html += "<tr><td class=\"meta\" colspan=\"2\">" + escaped + "</td></tr>";
        } else {
            html += "<tr><td class=\"ctx\">" + escaped + "</td><td class=\"ctx\">" + escaped + "</td></tr>";
        }
    }
...
void GitDiffDialog::rebuildWord()
{
    ...
    for (const auto& line : m_lines) {
        const bool hasWordMarkers = line.content.contains("{+") || line.content.contains("[-");
        if (line.prefix == '@' || line.prefix == 'd' || line.prefix == 'i' || line.prefix == 'h') {
            html += "<span class=\"meta\">" + htmlEscape(line.content) + "</span>\n";
        } else if (line.prefix == '+' || line.prefix == '-') {
            if (hasWordMarkers) {
                html += buildWordDiffLine(line.content) + "\n";
            } else {
                html += styleToken(line.content, line.prefix == '+' ? "add" : "del") + "\n";
            }
    ...
    for (const QString& rawLine : rawLines) {
        ...
        if (rawLine.startsWith("diff --git")) {
            ...
            line.content = rawLine;
                   ...
                   || rawLine.startsWith("deleted file") || rawLine.startsWith("--- ")
                   || rawLine.startsWith("+++ ") || rawLine.startsWith("rename ")
...
void GitDiffDialog::collectChangeBlocks()
{
    ...
    m_currentChange = m_changeBlocks.isEmpty() ? 0 : qMin(m_currentChange, m_changeBlocks.size() - 1);
}

void GitDiffDialog::scrollToChange(int index)
{
    if (!m_diffView || m_changeBlocks.isEmpty() || index < 0 || index >= m_changeBlocks.size()) {
        return;
    }
...
QString GitDiffDialog::resolveWordDiff()
{
    ...
    if (m_target == DiffTarget::Commit) {
        return m_git->executeWordDiff({"show", "--word-diff", "--color=never", "--pretty=format:", m_targetId});
    }
    QString relativePath = m_targetId;
    if (!m_git->repositoryPath().isEmpty() && relativePath.startsWith(m_git->repositoryPath())) {
        relativePath = relativePath.mid(m_git->repositoryPath().length() + 1);
    }
    if (m_staged) {
        return m_git->executeWordDiff({"diff", "--cached", "--word-diff", "--color=never", "--", relativePath});
    }
    return m_git->executeWordDiff({"diff", "--word-diff", "--color=never", "--", relativePath});
}
...
QString GitDiffDialog::styleToken(const QString& token, const QString& cssClass) const
{
    return QString("<span class=\"%1\">%2</span>").arg(cssClass, htmlEscape(token));
}
