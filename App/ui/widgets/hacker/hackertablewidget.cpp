...
HackerTableHeader::HackerTableHeader(Qt::Orientation orientation,
                                     QWidget *parent)
...
HackerTableWidget::HackerTableWidget(QWidget *parent) : HackerWidget(parent) {
  setupTable();
}
...
void HackerTableWidget::setupTable() {
  ...
  connect(m_table, &QTableWidget::cellClicked, this,
          &HackerTableWidget::cellClicked);
  connect(m_table, &QTableWidget::cellDoubleClicked, this,
          &HackerTableWidget::cellDoubleClicked);
  connect(m_table, &QTableWidget::currentCellChanged, this,
          &HackerTableWidget::currentCellChanged);
  connect(m_table, &QTableWidget::itemSelectionChanged, this,
          &HackerTableWidget::itemSelectionChanged);
}
...
void HackerTableWidget::setColumnCount(int columns) {
  m_table->setColumnCount(columns);
}
...
void HackerTableWidget::setItem(int row, int column, QTableWidgetItem *item) {
  m_table->setItem(row, column, item);
}
QTableWidgetItem *HackerTableWidget::item(int row, int column) const {
  return m_table->item(row, column);
}
void HackerTableWidget::setHorizontalHeaderLabels(const QStringList &labels) {
  m_table->setHorizontalHeaderLabels(labels);
}
void HackerTableWidget::setVerticalHeaderLabels(const QStringList &labels) {
  m_table->setVerticalHeaderLabels(labels);
}
void HackerTableWidget::setColumnWidth(int column, int width) {
  m_table->setColumnWidth(column, width);
}
void HackerTableWidget::setRowHeight(int row, int height) {
...
void HackerTableWidget::setSelectionMode(
    QAbstractItemView::SelectionMode mode) {
  m_table->setSelectionMode(mode);
}
void HackerTableWidget::setSelectionBehavior(
    QAbstractItemView::SelectionBehavior behavior) {
  m_table->setSelectionBehavior(behavior);
}
void HackerTableWidget::setAlternatingRowColors(bool enable) {
  m_table->setAlternatingRowColors(enable);
}
void HackerTableWidget::setSortingEnabled(bool enable) {
  m_table->setSortingEnabled(enable);
}

QSize HackerTableWidget::sizeHint() const { return QSize(400, 250); }
...
void HackerTableWidget::paintEvent(QPaintEvent *) {
  QPainter p(this);
