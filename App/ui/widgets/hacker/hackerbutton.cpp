...
HackerButton::HackerButton(const QIcon &icon, const QString &text,
                           QWidget *parent)
...
void HackerButton::setCheckable(bool checkable) { m_checkable = checkable; }
...
QSize HackerButton::minimumSizeHint() const { return QSize(48, 32); }
...
QColor HackerButton::hoverBgColor() const {
  ...
  switch (m_variant) {
  case Primary:
    return c.btnPrimaryHover;
  case Secondary:
    return c.btnSecondaryHover;
  case Danger:
    return c.btnDangerHover;
  case Ghost:
    return c.btnGhostHover;
  }
  return c.btnSecondaryHover;
}
...
QColor HackerButton::activeBgColor() const {
  ...
  switch (m_variant) {
  case Primary:
    return c.btnPrimaryActive;
  case Secondary:
    return c.btnSecondaryActive;
  case Danger:
    return c.btnDangerActive;
  case Ghost:
    return c.btnGhostActive;
  }
  return c.btnSecondaryActive;
}

void HackerButton::paintEvent(QPaintEvent *) {
  QPainter p(this);
  ...
  if (m_variant == Primary &&
      (m_hoverProgress > 0.01 || m_focusProgress > 0.01)) {
