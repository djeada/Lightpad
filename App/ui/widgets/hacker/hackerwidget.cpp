...
HackerWidget::HackerWidget(QWidget *parent) : QWidget(parent) {
...
void HackerWidget::setFocusProgress(qreal v) {
  if (qFuzzyCompare(m_focusProgress, v))
    return;
  m_focusProgress = v;
  update();
}
...
void HackerWidget::setPressProgress(qreal v) {
  if (qFuzzyCompare(m_pressProgress, v))
    return;
  m_pressProgress = v;
  update();
}
...
void HackerWidget::startAnimation(QPropertyAnimation *anim, qreal from,
                                  qreal to) {
  anim->stop();
