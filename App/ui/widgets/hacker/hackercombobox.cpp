...
HackerComboBox::HackerComboBox(QWidget *parent)
    : HackerWidget(parent)
{
    setupInnerCombo();
...
void HackerComboBox::setupInnerCombo()
{
    ...
    connect(m_combo, QOverload<int>::of(&QComboBox::currentIndexChanged),
            this, &HackerComboBox::currentIndexChanged);
    connect(m_combo, &QComboBox::currentTextChanged,
            this, &HackerComboBox::currentTextChanged);
...
void HackerComboBox::addItem(const QString &text, const QVariant &userData) { m_combo->addItem(text, userData); update(); }
void HackerComboBox::addItem(const QIcon &icon, const QString &text, const QVariant &userData) { m_combo->addItem(icon, text, userData); update(); }
void HackerComboBox::addItems(const QStringList &texts) { m_combo->addItems(texts); update(); }
void HackerComboBox::insertItem(int index, const QString &text, const QVariant &userData) { m_combo->insertItem(index, text, userData); update(); }
void HackerComboBox::removeItem(int index) { m_combo->removeItem(index); update(); }
void HackerComboBox::clear() { m_combo->clear(); update(); }
int HackerComboBox::currentIndex() const { return m_combo->currentIndex(); }
void HackerComboBox::setCurrentIndex(int index) { m_combo->setCurrentIndex(index); }
QString HackerComboBox::currentText() const { return m_combo->currentText(); }
int HackerComboBox::count() const { return m_combo->count(); }
QString HackerComboBox::itemText(int index) const { return m_combo->itemText(index); }
QVariant HackerComboBox::itemData(int index) const { return m_combo->itemData(index); }
...
QSize HackerComboBox::minimumSizeHint() const
{
    return QSize(80, 36);
}
...
{
    if (qFuzzyCompare(m_chevronRotation, r)) return;
    m_chevronRotation = r;
    update();
}

void HackerComboBox::paintEvent(QPaintEvent * /*event*/)
{
    QPainter p(this);
    ...
    // Border
    QColor borderColor = blendColors(c.inputBorder, c.borderFocus, m_focusProgress);
