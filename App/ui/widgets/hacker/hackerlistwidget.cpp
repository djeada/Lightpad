...
void HackerListDelegate::paint(QPainter *painter,
  ...
  QColor textColor =
      (option.state & QStyle::State_Selected) ? c.textPrimary : c.textPrimary;
...
HackerListWidget::HackerListWidget(QWidget *parent) : HackerWidget(parent) {
  setupList();
}
...
QListWidgetItem *HackerListWidget::item(int row) const {
...
QListWidgetItem *HackerListWidget::currentItem() const {
  return m_list->currentItem();
}
void HackerListWidget::clear() { m_list->clear(); }
QListWidgetItem *HackerListWidget::takeItem(int row) {
  return m_list->takeItem(row);
}
void HackerListWidget::insertItem(int row, const QString &text) {
  m_list->insertItem(row, text);
}
void HackerListWidget::insertItem(int row, QListWidgetItem *item) {
  m_list->insertItem(row, item);
}
void HackerListWidget::scrollToItem(const QListWidgetItem *item) {
  m_list->scrollToItem(item);
}

QSize HackerListWidget::sizeHint() const { return QSize(250, 200); }
...
void HackerListWidget::paintEvent(QPaintEvent *) {
  QPainter p(this);
