...
void HackerScrollBar::setPageStep(int step) {
  m_pageStep = step;
  update();
}
...
void HackerScrollBar::scheduleHide() { m_idleTimer->start(); }

void HackerScrollBar::paintEvent(QPaintEvent *) {
  if (m_barOpacity < 0.01 || m_max <= m_min)
    return;
...
void HackerScrollBar::mousePressEvent(QMouseEvent *event) {
  ...
  if (tr.contains(event->position())) {
    ...
    qreal pos = (m_orientation == Qt::Vertical) ? event->position().y()
                                                : event->position().x();
    qreal tl = trackLength();
    qreal th = thumbLength();
    qreal available = tl - th;
