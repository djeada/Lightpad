...
HackerSpinBox::HackerSpinBox(QWidget *parent)
    : HackerWidget(parent)
{
    setupInnerSpin();
...
void HackerSpinBox::setupInnerSpin()
{
    ...
    connect(m_spin, QOverload<int>::of(&QSpinBox::valueChanged),
            this, &HackerSpinBox::valueChanged);
...
void HackerSpinBox::setPrefix(const QString &prefix) { m_spin->setPrefix(prefix); }
void HackerSpinBox::setSuffix(const QString &suffix) { m_spin->setSuffix(suffix); }
void HackerSpinBox::setWrapping(bool wrapping) { m_spin->setWrapping(wrapping); }
...
void HackerSpinBox::resizeEvent(QResizeEvent *event)
{
    ...
    m_spin->setGeometry(hPad, vPad, width() - hPad - arrowW, m_spin->sizeHint().height());
}
...
void HackerSpinBox::paintEvent(QPaintEvent * /*event*/)
{
    QPainter p(this);
    ...
    // Border
    QColor borderColor = blendColors(c.inputBorder, c.inputBorderFocus, m_focusProgress);
    p.setPen(QPen(borderColor, 1.0));
    p.setBrush(Qt::NoBrush);
    p.drawRoundedRect(r, rad, rad);

    // Divider line before arrows
