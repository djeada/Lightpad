...
void HackerCheckBox::setCheckProgress(qreal v) {
  if (qFuzzyCompare(m_checkProgress, v))
    return;
  m_checkProgress = v;
  update();
}
...
void HackerCheckBox::paintEvent(QPaintEvent *) {
  QPainter p(this);
  ...
  QColor borderColor =
      blendColors(c.borderDefault, c.borderStrong, m_hoverProgress);
  borderColor = blendColors(borderColor, c.borderFocus, m_focusProgress);

  QColor fillColor =
      blendColors(Qt::transparent, c.accentPrimary, m_checkProgress);
...
void HackerCheckBox::toggle() { setChecked(!m_checked); }
