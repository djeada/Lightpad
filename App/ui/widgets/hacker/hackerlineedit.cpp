...
HackerLineEdit::HackerLineEdit(QWidget *parent) : HackerWidget(parent) {
  setupInnerEdit();
}
...
void HackerLineEdit::setupInnerEdit() {
  ...
  connect(m_edit, &QLineEdit::returnPressed, this,
          &HackerLineEdit::returnPressed);
  connect(m_edit, &QLineEdit::editingFinished, this,
          &HackerLineEdit::editingFinished);
...
void HackerLineEdit::setPlaceholderText(const QString &text) {
  m_edit->setPlaceholderText(text);
}
QString HackerLineEdit::placeholderText() const {
...
void HackerLineEdit::setEchoMode(QLineEdit::EchoMode mode) {
...
QSize HackerLineEdit::sizeHint() const { return QSize(200, 36); }

QSize HackerLineEdit::minimumSizeHint() const { return QSize(80, 36); }
...
void HackerLineEdit::resizeEvent(QResizeEvent *event) {
  ...
  m_edit->setGeometry(hPad, vPad, width() - 2 * hPad,
                      m_edit->sizeHint().height());
}
...
void HackerLineEdit::paintEvent(QPaintEvent *) {
  QPainter p(this);
  ...
  QColor borderColor =
      blendColors(c.inputBorder, c.accentPrimary, m_focusProgress);
  qreal borderY = r.bottom();
  QPen borderPen(borderColor, 2.0);
  p.setPen(borderPen);
  p.drawLine(QPointF(r.left() + rad, borderY),
             QPointF(r.right() - rad, borderY));
  ...
  QColor edgeBorder =
      blendColors(c.borderDefault, c.borderFocus, m_focusProgress);
