...
HackerTabBar::HackerTabBar(QWidget *parent) : HackerWidget(parent) {
...
void HackerTabBar::setTabsClosable(bool closable) {
  m_closable = closable;
  update();
}
...
void HackerTabBar::paintEvent(QPaintEvent *) {
  QPainter p(this);
  ...
  for (int i = 0; i < m_tabs.size(); ++i) {
    ...
    if (!tab.icon.isNull()) {
      QRect iconRect(contentX, (tr.height() - iconSize) / 2, iconSize,
                     iconSize);
    ...
    QRect textRect(contentX, tr.top(), tr.right() - contentX - closeSpace,
                   tr.height());
    QString elidedText =
        fm.elidedText(tab.text, Qt::ElideRight, textRect.width());
    ...
    if (m_closable && (isHovered || isCurrent)) {
      ...
      int m = 3;
      p.drawLine(cbr.topLeft() + QPoint(m, m),
                 cbr.bottomRight() - QPoint(m, m));
      p.drawLine(cbr.topRight() + QPoint(-m, m),
                 cbr.bottomLeft() + QPoint(m, -m));
    }
  }
