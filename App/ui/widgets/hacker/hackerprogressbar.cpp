...
HackerProgressBar::HackerProgressBar(QWidget *parent) : HackerWidget(parent) {
...
void HackerProgressBar::paintEvent(QPaintEvent *) {
  QPainter p(this);
  ...
  if (frac > 0.0) {
    ...
    if (m_scanEffect && m_scanOffset > 0.0 && m_scanOffset < 1.0) {
      ...
      p.setBrush(scanGrad);
      p.drawRect(
          QRectF(bandX - bandW, fillRect.top(), bandW * 2, fillRect.height()));
    }
