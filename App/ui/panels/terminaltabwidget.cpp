...
void TerminalTabWidget::applyTheme(const Theme &theme) {
  ...
  if (toolbar) {
    ...
    toolbar->setStyleSheet(
        ...
        QStringLiteral(
            "QWidget#terminalToolbar { background: %1; }"
            "QWidget#terminalToolbar QToolButton { color: %2; background: %1;"
            " border: 1px solid %3; border-radius: 3px; padding: 2px 8px; }"
            "QWidget#terminalToolbar QToolButton:hover { color: %1; "
            "background: %2; border-color: %2; }"
            "QWidget#terminalToolbar QToolButton:disabled { color: %3; }"
            "QToolButton::menu-indicator { image: none; width: 0; }")
            .arg(raised.name(), accent.name(), border.name()));
  }
...
void TerminalTabWidget::applyTabStyle(QTabWidget *tabWidget,
  ...
  QString tabWidgetStyle =
      QString("QTabWidget::pane {"
              ...
              "  background: %3;"
              "}")
          .arg(bgColor, textColor, accentColor, raisedColor);
