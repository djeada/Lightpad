...
Minimap::Minimap(QWidget* parent)
    ...
    , m_viewportColor(QColor(100, 149, 237, 60))  // Cornflower blue with transparency
...
void Minimap::setSourceEditor(QPlainTextEdit* editor)
{
    ...
    if (m_sourceEditor) {
        connect(m_sourceEditor, &QPlainTextEdit::textChanged, 
                this, &Minimap::onSourceTextChanged);
        connect(m_sourceEditor->verticalScrollBar(), &QScrollBar::valueChanged,
                this, &Minimap::onSourceScrollChanged);
        connect(m_sourceEditor, &QPlainTextEdit::cursorPositionChanged,
                this, &Minimap::onSourceCursorPositionChanged);
...
QPlainTextEdit* Minimap::sourceEditor() const
{
...
bool Minimap::isMinimapVisible() const
{
    return m_visible;
}
...
void Minimap::updateViewportRect()
{
    ...
    int visibleLineCount = m_sourceEditor->height() / m_sourceEditor->fontMetrics().lineSpacing();
...
void Minimap::scrollToY(int y)
{
    ...
    if (m_sourceEditor && lineNumber >= 0) {
        QTextBlock block = m_sourceEditor->document()->findBlockByNumber(lineNumber);
        if (block.isValid()) {
    ...
    while (block.isValid() && lineIndex * m_lineHeight < imageHeight) {
        ...
        if (layout && layout->lineCount() > 0) {
            ...
            for (int i = 0; i < text.length() && x < imageWidth; ++i) {
                ...
                painter.fillRect(QRectF(x, y, m_charWidth * 0.8, m_lineHeight * 0.7), charColor);
                x += m_charWidth;
                charIndex++;
            }
        } else {
